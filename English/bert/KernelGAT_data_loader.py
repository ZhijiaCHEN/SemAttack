...
class KGAT_Dataset(object):
    def __init__(self, data_path, label_map, tokenizer, sampleCnt=-1, max_len=130, evi_num=5, threshold=0.0):
        self.tokenizer = tokenizer
    ...
    def read_file(self, data_path):
        ...
        if self.sampleCnt > 0:
            data = random.sample(data, self.sampleCnt)
...
class DataLoaderTest(object):
    ...
    def __init__(self, data_path, label_map, tokenizer, args, cuda=True, batch_size=64, sample=-1):
        self.cuda = cuda
    ...
    def read_file(self, data_path):
        ...
        for instance in joblib.load(data_path):
            ...
            label = self.label_map[instance['label']]
            evi_list = evi_list[:self.evi_num]
            examples.append([evi_list, label, id])
        return examples
